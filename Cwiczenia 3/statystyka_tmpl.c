...
void print_histogram(const double tab[], size_t n, int x_start, double y_scale, char mark) {
    for (int i=0; i<n; i++){
        ...
        for(int j=0; j<round(tab[i]/y_scale); j++)
            printf("%c",mark);
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) {
    int winner_gate, choice;
    for (int i=0; i<n; i++){
        winner_gate = 1+rand()%3;
        choice = 1+rand()%3;
        if (winner_gate == choice)
            (*p_nonswitch_wins)+=1;
        else
            (*p_switch_wins)+=1;
    }
